...
class Grader:
    def check_console_output(topicId:int, tutorialId:int, code:str) -> Union[Literal[True], str]:
        if not (tutorial_data := TutorialDataLoader.find_tutorial(topicId=topicId, tutorialId=tutorialId)): raise TutorialDataNotFound
        ...
        prewritten_files = TutorialDataLoader.get_prewritten_cpp_files(topicId=topicId, tutorialId=tutorialId)
        compile_result = CPP_Compiler.write_compile_run(code=code, add_custom_headers=False, other_files=prewritten_files)
    ...
    def check_doctest(topicId:int, tutorialId:int, code: str) -> Union[Literal[True], str]:
        ...
        prewritten_tests = TutorialDataLoader.get_prewritten_test_files(topicId=topicId, tutorialId=tutorialId)
        if not prewritten_tests: return True
        
        prewritten_files = TutorialDataLoader.get_prewritten_cpp_files(topicId=topicId, tutorialId=tutorialId)
        
        process_result = CPP_Compiler.write_compile_run(code=code, add_custom_headers=True, other_files=[*prewritten_files, *prewritten_tests])
