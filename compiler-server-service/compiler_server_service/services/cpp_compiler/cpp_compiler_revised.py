...
class CPP_Compiler:
    ...
    def write_compile_run(cls, all_code: Union[str, LogicalCodeFile, List[LogicalCodeFile]], other_files: List[Union[str, Path]] = (), add_custom_headers: bool = False, werrrors=True):
        ...
        if isinstance(all_code, str):
            all_code = [LogicalCodeFile(code=all_code, filename=None)]
        elif isinstance(all_code, LogicalCodeFile):
            all_code = [all_code]
        elif isinstance(all_code, list):
            if all(isinstance(a, LogicalCodeFile) for a in all_code):
                pass
            elif all(isinstance(a, str) for a in all_code):
                [LogicalCodeFile(code=i, filename=None) for i in all_code]

        else:
            raise ValueError(type(all_code))

        with tempfile.TemporaryDirectory(dir=USER_TEMP_FILES_DIR_PATH) as tmp_dir_path:
            result = cls.write_and_compile(code_files=all_code, other_files=other_files, temp_dir_path=tmp_dir_path,
                                           executable_filepath='output.exe', add_custom_headers=add_custom_headers, werrors=werrrors)
    ...
    def write_and_compile(cls, code_files: Union[str, LogicalCodeFile, List[LogicalCodeFile]], temp_dir_path: str, other_files: List[Union[str, Path]] = (),
        ...
        if isinstance(code_files, str):
            code_files = [LogicalCodeFile(code=code_files, filename=None)]
        elif isinstance(code_files, LogicalCodeFile):
            code_files = [code_files]
        elif isinstance(code_files, list):
            if all(isinstance(a, LogicalCodeFile) for a in code_files):
                pass
            elif all(isinstance(a, str) for a in code_files):
                [LogicalCodeFile(code=i, filename=None) for i in code_files]

        else:
            raise ValueError(type(code_files))

        if not isinstance(other_files, Iterable):
        ...
        compile_result = compile_function(
            *temp_files, *other_files, out_filepath=executable_filepath, werrors=werrors)
    ...
    def compile_files(cls, *files_to_be_compiled, out_filepath: Path, werrors=True):
        ...
        # error_options = []
        error_options = ['-Wall', '-Weffc++', '-Wextra',
                         "-Wsign-conversion"] if werrors else []

        return CompilationResult(
            ProcessWrapper.shell_run(
                'g++', *files_to_be_compiled, '-o', out_filepath, *error_options),
