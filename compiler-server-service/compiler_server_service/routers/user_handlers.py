...
def login(request: Request, data: POST__Login):
    if found_user := UserData.find_by_name(data.username):
        ...
        return {'user_id': found_user.id,
                ...
                'privilege': found_user.privilege}
    else:
...
def create_user(request: Request, data: POST_Create_User):
    ...
    if not new_user:
        raise HTTPException(status_code=500, detail='error on user creation')
    return {'user_id': new_user.id, 'username': new_user.name, 'privilege': new_user.privilege}
