...
class DELETE_Delete_Exam_Solution(BaseModel):
    user_id: str


@router.delete('/{id}', status_code=200)
def delete_exam_solution_by_id(request: Request, id: str, data: DELETE_Delete_Exam_Solution):
    ...
    found_resource = ExamSolutionData.find_by_id(id)
    if not found_resource:
        raise HTTPException(
            status_code=404, detail='Exam solution ID does not exist')
    ...
    try:
        with client.start_session() as session:
            with session.start_transaction():
                deleted_count = ExamSolutionData.remove_by_id(id)
                if deleted_count == 0:
                    raise HTTPException(
    ...
    return {
        'detail': 'Exam solution successfully deleted',
        'id': id,
