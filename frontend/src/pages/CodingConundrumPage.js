...
export default function CodingConumdrumPage() {
  ...
  class GameStateManager {
    constructor(user) {
      this.user = user;
      this.connection = GameDataService.startConnection(
        this.websocketIncomingMessageHandler
      );
      this.connection.onerror = (e) => {
        console.log(e);
        alert(
          "Could not establish a connection with the compiler server. Please refresh the page or try again later."
        );
      };
    }
    websocketIncomingMessageHandler(event) {
      ...
      if (data["RoundCreatedMessage"]) {
        ...
        setGamePlayers(
          (i) => new Set([...i, new UserData(joined_user, "", "", "")])
        );
  ...
  function initiateGameState() {
    ...
    let userGlobalPoints =
      UserDataService.getUserDataValue(USER_POINTS_KEY_NAME);
      ...
      <Grid container mt={8}>
        ...
        >
          ...
          >
            ...
            <Box
              sx={{ height: "40vh", p: 5, overflowY: "scroll" }}
              key={reactComponentKey()}
            >
              <Typography
              ...
              {!gptText && (
                <Box>
                  ...
                  <Typography fontFamily="Open Sans" sx={{ pt: 1 }}>
                    When a new round starts, ChatGPT will generate a scenario
                    for which you must create code that best fits the prompt.
                  </Typography>
                  ...
                  <Typography fontFamily="Open Sans" sx={{ pt: 0 }}>
                    You can compile your code as many times as you wish, but can
                    only send it for evaluation once.
                  </Typography>
            ...
            <Box key={reactComponentKey()}>
              ...
              <TabPanel value={tabValue} index={0}>
                ...
                >
                  {channelTextData.map((i) => {
                    return (
                      <Box mr={5} minHeight="1.3rem" key={reactComponentKey()}>
                        <Typography
                          align="center"
                          fontFamily="Segoe UI"
                          key={reactComponentKey()}
              ...
              <TabPanel value={tabValue} index={1}>
                <Box
                  sx={{ pl: 5, height: "20vh", overflowY: "auto" }}
                  key={reactComponentKey()}
                >
                  {Array.from(gamePlayers).map((data) =>
                    userAvatarDisplay(data, reactComponentKey())
                  )}
                </Box>
              </TabPanel>
            </Box>
  ...
  return (
    <Stack direction="row" spacing={2} key={key}>
      <Tooltip title={userData.username}>
        <Avatar sx={{ mb: 2, bgcolor: grey[900] }}>
          <AccessibilityNewIcon />
        </Avatar>
      </Tooltip>
      <Chip
        label={`${userData.roundPoints} points | ${userData.totalPoints} total`}
        variant="outlined"
      />
    </Stack>
  );
...
const TimerComponent = forwardRef((props, ref) => {
  ...
  return (
    <Stack direction="column">
      <Typography fontFamily="PT Serif" ml={5} key="timeleft">
        Time remaining this round:
      </Typography>
      <Typography fontFamily="Segoe UI" variant="h5" ml={5} key="timeleft2">
