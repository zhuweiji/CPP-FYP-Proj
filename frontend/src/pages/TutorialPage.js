...
export default function TutorialPage(props) {
  ...
  const [previousTutorialDisabled, setPreviousTutorialDisabled] =
    useState(false);
  ...
  const { tutorialId, topicId } = useParams();
  if (!tutorialId) console.error("tutorialId of this page could not be found!");
  if (!topicId) console.error("topicId of this page could not be found!");
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      tutorialData = await TutorialDataFetch.getTutorialInformation(
        topicId,
        tutorialId
      );
      ...
      let [prevTopicId, prevTutorialId] =
        tutorialData["previous_tutorial_topicid_tutid"];
      ...
      let [nextTopicId, nextTutorialId] =
        tutorialData["next_tutorial_topicid_tutid"];
      ...
      if (tutorialData.no_tutorial === true) {
        navigate(`/notebook/${nextTopicId}/${nextTutorialId}`, {
          replace: true,
        });
      }
    }
        ...
        <Grid item xs={8} mt={8}>
          {initialEditorData && (
            <CodeEditor
              ...
              files={initialEditorData}
              includeGradeButton={true}
              updateGradingToPassed={() => {
